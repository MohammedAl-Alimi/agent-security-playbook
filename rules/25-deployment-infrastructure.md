...
Next.js: don't set `productionBrowserSourceMaps: true` unless sources are meant to be public, and never expose server source maps. Seed/debug routes (`/api/seed`, `/api/debug`) must be excluded from prod builds, not merely hidden.

**Verify:** pre-deploy check — `curl -s -o /dev/null -w '%{http_code}' https://$HOST/docs /openapi.json /_next/static/**/*.map` → 401/404 for each; grep for `debug=True`/`DEBUG=true` in prod config → zero.
